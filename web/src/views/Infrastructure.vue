<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Infrastructure Overview</h1>

    <!-- Nodes Table -->
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden mb-8">
      <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white">Cluster Nodes</h3>
      </div>
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead class="bg-gray-50 dark:bg-gray-900">
          <tr>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Node Name</th>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Pods</th>
          </tr>
        </thead>
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          <tr v-for="node in nodes" :key="node.name">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ node.cpu }}%</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ node.memory }}%</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ node.pods }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="service in services" :key="service.name" class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        ...
        <dl class="space-y-2">
          <div class="flex justify-between text-sm">
            ...
            <dd class="text-gray-900 dark:text-white font-medium">{{ service.replicas }}</dd>
          </div>
          <div class="flex justify-between text-sm">
            <dt class="text-gray-500 dark:text-gray-400">Port</dt>
            <dd class="text-gray-900 dark:text-white font-medium">{{ service.port }}</dd>
          </div>
          <div class="flex justify-between text-sm">
            <dt class="text-gray-500 dark:text-gray-400">Version</dt>
            <dd class="text-gray-900 dark:text-white font-medium">{{ service.version }}</dd>
          </div>
        </dl>
      </div>
    </div>
