...
class AnomalyDetector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Model parameters
        self.contamination = config.get('anomaly_detection', {}).get('contamination', 0.1)
    ...
    async def detect_anomalies(self, metrics_batch: List[Dict]) -> List[AnomalyResult]:
        ...
        try:
            ...
            self.logger.debug(f"Detected {len(anomalies)} anomalies from {len(metrics_batch)} metrics")
            return anomalies
    ...
    async def _detect_with_ensemble(self, features_df: pd.DataFrame, original_metrics: List[Dict]) -> List[AnomalyResult]:
        ...
        try:
            ...
            ensemble_anomalies = self._combine_predictions(
                predictions, scores, features_df, original_metrics, statistical_anomalies
            )
        ...
        try:
            ...
            if features_scaled.shape[1] > 10:  # Only apply PCA if many features
                if hasattr(self.pca_transformer, 'components_'):
                    ...
                    features_scaled = self.pca_transformer.fit_transform(features_scaled)
    ...
    def _create_anomaly_result(
        ...
        try:
            ...
            return AnomalyResult(
                ...
                type=anomaly_type,
                severity=severity,
                description=self._generate_anomaly_description(original_metric, anomaly_type),
    ...
    def _generate_anomaly_description(self, metric: Dict, anomaly_type: str) -> str:
        ...
        return descriptions.get(anomaly_type, f"Anomaly detected in {metric_name} for service {service}")
        ...
        try:
            ...
            if features.shape[1] > 0:
                self.algorithms['autoencoder'] = AutoEncoder(
                    hidden_neurons=[features.shape[1] // 2, features.shape[1] // 4, features.shape[1] // 2],
                    contamination=self.contamination,
                    epochs=50,
                    verbose=0
                )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'algorithms_loaded': len([alg for alg in self.algorithms.values() if alg is not None]),
